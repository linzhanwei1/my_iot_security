...
void app_main(void) {
  ...
  printf("  1. dh generate 2048 bit prime(G, P) ... ok\n");

  ret = mbedtls_dhm_make_public(&dhm_srv, 256, srv_pub, sizeof(srv_pub),
                                mbedtls_ctr_drbg_random, &ctr_drbg);
  assert_exit(ret == 0, ret);
  dump_buf("  2. dh server generate public parameter:", srv_pub,
           sizeof(srv_pub));

  ret = mbedtls_dhm_make_public(&dhm_cli, 256, cli_pub, sizeof(cli_pub),
                                mbedtls_ctr_drbg_random, &ctr_drbg);
  assert_exit(ret == 0, ret);
  dump_buf("  3. dh client generate public parameter:", cli_pub,
           sizeof(cli_pub));

  ret = mbedtls_dhm_read_public(&dhm_srv, cli_pub, sizeof(cli_pub));
  assert_exit(ret == 0, ret);
  printf("  4. dh erver read public ... ok\n");

  ret = mbedtls_dhm_read_public(&dhm_cli, srv_pub, sizeof(srv_pub));
  assert_exit(ret == 0, ret);
  printf("  5. dh client read public ... ok\n");

  ret = mbedtls_dhm_calc_secret(&dhm_srv, srv_secret, sizeof(srv_secret), &n,
                                mbedtls_ctr_drbg_random, &ctr_drbg);
  assert_exit(ret == 0, ret);
  dump_buf("  6. dh erver generate secret:", srv_secret, sizeof(srv_secret));

  ret = mbedtls_dhm_calc_secret(&dhm_cli, cli_secret, sizeof(cli_secret), &n,
                                mbedtls_ctr_drbg_random, &ctr_drbg);
  assert_exit(ret == 0, ret);
  dump_buf("  7. dh client generate sectet:", cli_secret, sizeof(cli_secret));
