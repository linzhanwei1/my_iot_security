...
#define SERVER_ADDR "182.92.148.82"
#define SERVER_PORT "5200"
#define HOSTNAME "182.92.148.82"
...
void app_main(void) {
  ...
  ret = mbedtls_net_connect(&ctx, SERVER_ADDR, SERVER_PORT,
  ...
  printf(" ok\n > Write to server:");
  ret = mbedtls_ssl_write(&ssl, (const uint8_t *)GET_REQUEST,
                          strlen(GET_REQUEST));
  assert_exit(ret > 0, ret);
